...
рассчитайте как взвешенное арифметическое показателей всех моделей, взяв веса
для первой и второй модели как 3/8, а для третьей и четвертой - как 1/8.

Загрузите данные решения, посчитайте значение энергопотребления для требуемых
дат для тех зданий, которые посчитаны в модели, и выгрузите результат в
виде CSV-файла (submission.csv).

Сколько строк в итоговом файле имеют ненулевое значение для
показателя meter_reading ?
...
def weather_interpolate_diff(weather):
    ...
    weather.at[0, "air_temperature_diff1"] = weather.at[
        1, "air_temperature_diff1"]
    weather["air_temperature_diff2"] = weather["air_temperature_diff1"].diff()
    weather.at[0, "air_temperature_diff2"] = weather.at[
        1, "air_temperature_diff2"]
    return weather
...
def calculate_model_coeffs(columns):
    ...
    for building in buildings:
        ...
        for hour in hours:
            ...
            coeffs[building][hour] = np.append(
                coeffs[building][hour], model.intercept_)
    return coeffs
...
только 50 первых зданий"""

# Потребуется ~6 Гб оперативной памяти. Если память компьютера ограничена,
# имеет смысл провести расчеты по частям: например, по 10 зданий


def calculate_model(x, model, columns):
    return (
            np.sum([x[col] * model[i] for i, col in enumerate(columns[3:])]
                   )
            + model[len(columns) - 3]
    )
...
def calculate_ensemble(x):
    lr = -1
    lr_w = calculate_model(
        x,
        energy_lr_w[x.building_id][x.hour],
        lr_columns_weather
    )
    lr_d = calculate_model(
        x,
        energy_lr_d[x.building_id][x.hour],
        lr_columns_days
    )
    lr_ww = calculate_model(
        x,
        energy_lr_ww[x.building_id][x.hour],
        lr_columns_weeks
    )
    lr_m = calculate_model(
        x,
        energy_lr_m[x.building_id][x.hour],
        lr_columns_monthes
    )
...
"""Усечение данных до требуемого формата: row_id,
meter_reading - и выгрузка результата"""

results_ready = pd.DataFrame(
    results,
    columns=[
        "row_id",
        "meter_reading"
