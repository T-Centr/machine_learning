...
Разберем архитектуру GoogleLeNet для решения задач распознавания изображений.
...
def load_x(df):
    ...
    for j, file in enumerate(df["Image"]):
        img = image.load_img(
            os.path.join(filesDir, file), target_size=(224, 224)
        )
...
# Создание модели GoogLeNet. Источник:
...
def create_googlenet(weights_path=None):
    ...
    googlenet = Model(
        inputs=input,
        outputs=[
            loss1_classifier_final,
            loss2_classifier_final,
